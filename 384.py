...
class AS_ECDSA_384:
    def __init__(self):
        ...
        self.generator = NIST384p.generator
    ...
    def _circuit(self, stmt, wit):
        """
        ZKP Circuit for P-384:
        Verifies:
        1. pk derived from seed via SHA-384 KDF
        2. signature constructed using nonce c = (k + seed) mod q
        """
        pk_bytes, m, (r, s) = stmt
        seed, k = wit

        # [Logic 1] Re-derive SK (Using SHA-384)
        sk_int = int.from_bytes(self.hash_func(b"AS-KDF-P384:" + seed).digest(), 'big') % self.order
    ...
    def seed_gen(self):
        # [Change] P-384 建议使用 48 字节 (384 bits) 种子
    ...
    # [4] AS.SignAuth (Strictly following Figure 3 for P-384)
    def sign_auth(self, pp, seed, m):
        ...
        # Step 3: Select random k (48 bytes for P-384)
        k = int.from_bytes(os.urandom(48), 'big') % self.order
...
if __name__ == "__main__":
    ...
    msg = b"P-384 High Security Test Message"
